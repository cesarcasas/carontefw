<?
	...
	* @package system.db
	*/
	class PgSQLConnection extends Main implements DBConnection
	{
		/**
		 * @var integer
		 */
		...
		public function connect($host, $user, $pass, $db = '')
		{
			...
			$string = "host={$host} port={$this->port} dbname={$db} user={$user} password={$pass}";
			try {
				$link = pg_pconnect($string);
			} catch (Exception $e) {
				throw new SQLException(pg_last_error(), 0);
			} catch (RuntimeException $e) {
				throw new SQLException(pg_last_error(), 1);
			} 
